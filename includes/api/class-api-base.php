...
protected function parse_json_response( $response ) {
	...
	error_log( 'Parsing AI Response: ' . substr( $response, 0, 500 ) );
	
	// Try to extract JSON from response (handles cases where AI adds explanation text)
	preg_match( '/\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}/s', $response, $matches );
	
	if ( empty( $matches[0] ) ) {
		error_log( 'No JSON found in response: ' . $response );
	...
	error_log( 'Extracted JSON: ' . $json_string );
	
	$data = json_decode( $json_string, true );
	
	if ( json_last_error() !== JSON_ERROR_NONE ) {
		error_log( 'JSON Parse Error: ' . json_last_error_msg() );
		error_log( 'Failed JSON String: ' . $json_string );
		return new WP_Error( 'json_error', __( 'Invalid JSON in API response: ', 'ai-seo-pro' ) . json_last_error_msg() );
	}
	...
	error_log( 'Successfully parsed meta data: ' . print_r( $data, true ) );
	...
	protected function make_request( $url, $headers, $body ) {
		...
		if ( $status_code !== 200 ) {
			...
			$error_message = isset( $error_data['error']['message'] ) 
				...
				: sprintf( __( 'API request failed with status code: %d', 'ai-seo-pro' ), $status_code );
