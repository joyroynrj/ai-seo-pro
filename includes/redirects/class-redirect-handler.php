...
class AI_SEO_Pro_Redirect_Handler
{
    ...
    public function handle_redirect_early()
    {
        ...
        // Debug logging
        error_log('AI SEO Pro Redirect - Current URL: ' . $current_url);

        // Normalize URL for matching
        $normalized_url = $this->normalize_url_for_matching($current_url);

        error_log('AI SEO Pro Redirect - Normalized URL: ' . $normalized_url);

        // Find matching redirect
        $redirect = $this->manager->find_redirect($normalized_url);

        if (!$redirect) {
            error_log('AI SEO Pro Redirect - No redirect found for: ' . $normalized_url);
            return;
        }

        error_log('AI SEO Pro Redirect - Found redirect: ' . print_r($redirect, true));
        ...
        error_log('AI SEO Pro Redirect - Redirecting to: ' . $target_url . ' (Type: ' . $redirect->redirect_type . ')');
    ...
    private function perform_redirect($target_url, $redirect_type)
    {
        ...
        switch ($redirect_type) {
            ...
            case '410':
                ...
                nocache_headers();
                wp_die(
                    __('This content has been permanently removed.', 'ai-seo-pro'),
                    __('Gone', 'ai-seo-pro'),
                    array('response' => 410)
                );
                exit;
            case '451':
                ...
                nocache_headers();
                wp_die(
                    __('This content is unavailable for legal reasons.', 'ai-seo-pro'),
                    __('Unavailable For Legal Reasons', 'ai-seo-pro'),
                    array('response' => 451)
                );
                exit;
            default:
