<?php
/**
 * SEO Analyzer - Returns structured analysis with problems and good results.
 *
 * @package    AI_SEO_Pro
...
class AI_SEO_Pro_SEO_Analyzer
{
    ...
    public function analyze($post_id, $content = '', $title = '', $focus_keyword = '', $meta_title = '', $meta_description = '', $slug = '')
    {
        ...
        if (!empty($focus_keyword) && !empty($meta_title)) {
            if (stripos($meta_title, $focus_keyword) !== false) {
                ...
                if (stripos($meta_title, $focus_keyword) === 0) {
                    $good[] = array(
        ...
        if (!empty($focus_keyword) && !empty($slug)) {
            ...
            if (stripos($slug_check, $keyword_lower) !== false) {
                $good[] = array(
        ...
        if (!empty($focus_keyword) && $word_count > 0) {
            ...
            if ($density >= 0.5 && $density <= 2.5) {
                $good[] = array(
                    ...
                    'message' => sprintf(__('The focus keyphrase appears %.1f%% of the time. Good job!', 'ai-seo-pro'), $density),
                );
            } elseif ($density < 0.5) {
                $problems[] = array(
                    'title' => __('Keyphrase density', 'ai-seo-pro'),
                    'message' => sprintf(__('The focus keyphrase appears %.1f%% of the time. Try to use it more often.', 'ai-seo-pro'), $density),
                );
            } else {
                $problems[] = array(
                    'title' => __('Keyphrase density', 'ai-seo-pro'),
                    'message' => sprintf(__('The focus keyphrase appears %.1f%% of the time. This might be keyword stuffing.', 'ai-seo-pro'), $density),
                );
            }
        }
        ...
        if (!empty($meta_description)) {
            ...
            if ($desc_length >= 120 && $desc_length <= 160) {
                ...
                $problems[] = array(
                    'title' => __('Meta description length', 'ai-seo-pro'),
                    'message' => sprintf(__('The meta description is too short (%d characters). Use at least 120 characters.', 'ai-seo-pro'), $desc_length),
                );
            } else {
                $problems[] = array(
                    'title' => __('Meta description length', 'ai-seo-pro'),
                    'message' => sprintf(__('The meta description is too long (%d characters). Keep it under 160 characters.', 'ai-seo-pro'), $desc_length),
                );
            }
        } else {
            $problems[] = array(
        ...
        if (!empty($meta_title)) {
            ...
            if ($title_length >= 50 && $title_length <= 60) {
                ...
                $problems[] = array(
                    'title' => __('SEO title width', 'ai-seo-pro'),
                    'message' => sprintf(__('The SEO title is too short (%d characters). Use between 50-60 characters.', 'ai-seo-pro'), $title_length),
                );
            } else {
                $problems[] = array(
                    'title' => __('SEO title width', 'ai-seo-pro'),
                    'message' => sprintf(__('The SEO title is too long (%d characters). Keep it under 60 characters.', 'ai-seo-pro'), $title_length),
                );
            }
        }
        ...
        if ($word_count >= 300) {
            $good[] = array(
                ...
                'message' => sprintf(__('The text contains %d words. Good job!', 'ai-seo-pro'), $word_count),
            );
        } else {
            $problems[] = array(
                'title' => __('Text length', 'ai-seo-pro'),
                'message' => sprintf(__('The text contains %d words. Add more content to improve SEO (recommended: 300+ words).', 'ai-seo-pro'), $word_count),
            );
        }
        ...
        if ($h1_count === 1) {
            ...
            $problems[] = array(
                'title' => __('Single title', 'ai-seo-pro'),
                'message' => sprintf(__('You have %d H1 headings. Use only one H1 heading per page.', 'ai-seo-pro'), $h1_count),
            );
        }
        ...
        if (!empty($focus_keyword)) {
            ...
            if ($total_subheadings > 0 && $keyword_in_headings > 0) {
                ...
                if ($percentage >= 30 && $percentage <= 75) {
                    $good[] = array(
                        ...
                        'message' => sprintf(__('%d of your %d subheadings contain the focus keyphrase. Good job!', 'ai-seo-pro'), $keyword_in_headings, $total_subheadings),
                    );
            ...
            } elseif ($total_subheadings > 0 && $keyword_in_headings === 0) {
                $problems[] = array(
                    'title' => __('Keyphrase in subheading', 'ai-seo-pro'),
        ...
        if ($internal_links >= 1) {
            $good[] = array(
                ...
                'message' => sprintf(__('You have %d internal link(s). Good job!', 'ai-seo-pro'), $internal_links),
            );
        } else {
            $problems[] = array(
                'title' => __('Internal links', 'ai-seo-pro'),
        ...
        if ($external_links >= 1) {
            $good[] = array(
                ...
                'message' => sprintf(__('You have %d outbound link(s). Good job!', 'ai-seo-pro'), $external_links),
            );
        } else {
            $problems[] = array(
                'title' => __('Outbound links', 'ai-seo-pro'),
        ...
        if (!empty($focus_keyword)) {
            ...
            if ($competing_links === 0) {
                ...
                $problems[] = array(
                    'title' => __('Competing links', 'ai-seo-pro'),
                    'message' => sprintf(__('%d link(s) use your keyphrase as anchor text. This could cause competition.', 'ai-seo-pro'), $competing_links),
                );
            }
        }
        ...
        if ($total_images > 0) {
            ...
            foreach ($images[0] as $img) {
                if (preg_match('/alt=["\']([^"\']*)["\']/', $img, $alt_match)) {
                    if (!empty(trim($alt_match[1]))) {
                        $images_with_alt++;
                        if (!empty($focus_keyword) && stripos($alt_match[1], $focus_keyword) !== false) {
            ...
            if ($images_with_alt === $total_images) {
                ...
                $problems[] = array(
                    'title' => __('Images', 'ai-seo-pro'),
                    'message' => sprintf(__('%d image(s) are missing alt text. Add descriptive alt text to improve accessibility and SEO.', 'ai-seo-pro'), $missing),
                );
            }
            ...
            if (!empty($focus_keyword)) {
                if ($images_with_keyword_alt > 0) {
                    $good[] = array(
                        ...
                        'message' => sprintf(__('%d image(s) contain the focus keyphrase in the alt text. Good job!', 'ai-seo-pro'), $images_with_keyword_alt),
                    );
                } else {
                    $problems[] = array(
                        'title' => __('Keyphrase in image alt attributes', 'ai-seo-pro'),
    ...
    private function count_competing_links($content, $keyword)
    {
        ...
        foreach ($links[1] as $anchor) {
            ...
            if (stripos($anchor_text, $keyword) !== false) {
                $count++;
            }
        }
