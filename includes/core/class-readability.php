	...
	private function get_recommendations( $analysis ) {
		...
		if ( $analysis['avg_sentence_length'] > 20 ) {
			$recommendations[] = array(
				'type'    => 'warning',
				'message' => sprintf(
					__( 'Average sentence length is %.1f words. Try to use shorter sentences (under 20 words).', 'ai-seo-pro' ),
		...
		if ( $analysis['long_sentences'] > 3 ) {
			$recommendations[] = array(
				'type'    => 'info',
				'message' => sprintf(
					__( '%d sentences are very long. Consider breaking them up.', 'ai-seo-pro' ),
