...
$sitemap_enabled       = get_option( 'ai_seo_pro_sitemap_enabled', true );
$include_images        = get_option( 'ai_seo_pro_sitemap_include_images', true );
$include_taxonomies    = get_option( 'ai_seo_pro_sitemap_include_taxonomies', true );
$include_authors       = get_option( 'ai_seo_pro_sitemap_include_authors', false );
$entries_per_page      = get_option( 'ai_seo_pro_sitemap_entries_per_page', 1000 );
$ping_search_engines   = get_option( 'ai_seo_pro_sitemap_ping_search_engines', true );
$saved_post_types      = get_option( 'ai_seo_pro_sitemap_post_types', array() );
$saved_taxonomies      = get_option( 'ai_seo_pro_sitemap_taxonomies', array() );
...
if ( empty( $saved_post_types ) ) {
	$saved_post_types = array_keys( $post_types );
}

// Default taxonomies if not set
if ( empty( $saved_taxonomies ) ) {
	$saved_taxonomies = array_keys( $taxonomies );
}
...
<div class="wrap ai-seo-pro-settings">
		...
		<?php if ( $sitemap_enabled ) : ?>
			<div class="sitemap-enabled">
				...
				<p>
					...
					<br>
					<a href="<?php echo esc_url( $sitemap_url ); ?>" target="_blank" class="sitemap-url">
				...
				<div class="sitemap-actions">
					...
					<button type="button" class="button" id="copy-sitemap-url" data-url="<?php echo esc_url( $sitemap_url ); ?>">
						<?php esc_html_e( 'Copy URL', 'ai-seo-pro' ); ?>
					</button>
				</div>
			</div>
		<?php else : ?>
	...
	<form method="post" action="options.php">
			...
			<table class="form-table">
				<tr>
					...
					<td>
						<label class="ai-seo-toggle">
							<input type="checkbox" 
								   id="ai_seo_pro_sitemap_enabled" 
								   name="ai_seo_pro_sitemap_enabled" 
								   value="1" 
				...
				<tr>
					...
					<td>
						<select id="ai_seo_pro_sitemap_entries_per_page" name="ai_seo_pro_sitemap_entries_per_page">
							...
							<option value="1000" <?php selected( $entries_per_page, 1000 ); ?>>1000</option>
							<option value="2500" <?php selected( $entries_per_page, 2500 ); ?>>2500</option>
						</select>
						<p class="description">
				...
				<tr>
					...
					<td>
						<label class="ai-seo-toggle">
							<input type="checkbox" 
								   id="ai_seo_pro_sitemap_include_images" 
								   name="ai_seo_pro_sitemap_include_images" 
								   value="1" 
				...
				<tr>
					...
					<td>
						<label class="ai-seo-toggle">
							<input type="checkbox" 
								   id="ai_seo_pro_sitemap_ping_search_engines" 
								   name="ai_seo_pro_sitemap_ping_search_engines" 
								   value="1" 
								   <?php checked( $ping_search_engines ); ?>>
							<span class="slider"></span>
						</label>
						<p class="description">
		...
		<div class="ai-seo-pro-card">
			...
			<table class="form-table">
				<?php foreach ( $post_types as $post_type ) : ?>
					<tr>
						<th scope="row">
							<label for="sitemap_pt_<?php echo esc_attr( $post_type->name ); ?>">
						...
						<td>
							<label class="ai-seo-toggle">
								<input type="checkbox" 
									   id="sitemap_pt_<?php echo esc_attr( $post_type->name ); ?>" 
							...
							<span class="post-count">
								...
								printf( esc_html__( '%d published', 'ai-seo-pro' ), $published );
								?>
							</span>
						</td>
					</tr>
		...
		<div class="ai-seo-pro-card">
			...
			<table class="form-table">
				<tr>
					...
					<td>
						<label class="ai-seo-toggle">
							<input type="checkbox" 
								   id="ai_seo_pro_sitemap_include_taxonomies" 
								   name="ai_seo_pro_sitemap_include_taxonomies" 
								   value="1" 
								   <?php checked( $include_taxonomies ); ?>>
							<span class="slider"></span>
						</label>
						<p class="description">
			...
			<div class="taxonomy-list" <?php echo ! $include_taxonomies ? 'style="display:none;"' : ''; ?>>
				<table class="form-table">
					<?php foreach ( $taxonomies as $taxonomy ) : ?>
						<tr>
							<th scope="row">
								<label for="sitemap_tax_<?php echo esc_attr( $taxonomy->name ); ?>">
							...
							<td>
								<label class="ai-seo-toggle">
									<input type="checkbox" 
										   id="sitemap_tax_<?php echo esc_attr( $taxonomy->name ); ?>" 
										   name="ai_seo_pro_sitemap_taxonomies[]" 
										   value="<?php echo esc_attr( $taxonomy->name ); ?>" 
										   <?php checked( in_array( $taxonomy->name, $saved_taxonomies, true ) ); ?>>
									<span class="slider"></span>
								</label>
								<?php
								...
								<span class="post-count">
									...
									printf( esc_html__( '%d terms', 'ai-seo-pro' ), is_wp_error( $term_count ) ? 0 : $term_count );
									?>
								</span>
							</td>
						</tr>
			...
			<table class="form-table">
				<tr>
					...
					<td>
						<label class="ai-seo-toggle">
							<input type="checkbox" 
								   id="ai_seo_pro_sitemap_include_authors" 
								   name="ai_seo_pro_sitemap_include_authors" 
								   value="1" 
								   <?php checked( $include_authors ); ?>>
							<span class="slider"></span>
						</label>
						<p class="description">
						...
						<?php
						$author_count = count( get_users( array(
						...
						<span class="post-count">
							...
							printf( esc_html__( '%d authors with published posts', 'ai-seo-pro' ), $author_count );
							?>
						</span>
					</td>
				</tr>
...
jQuery(document).ready(function($) {
	...
	$('#copy-sitemap-url').on('click', function() {
		...
		navigator.clipboard.writeText(url).then(function() {
			alert('<?php esc_html_e( 'Sitemap URL copied to clipboard!', 'ai-seo-pro' ); ?>');
		});
	});
	...
	$('#flush-sitemap-cache').on('click', function() {
		var $btn = $(this);
		$btn.prop('disabled', true).text('<?php esc_html_e( 'Flushing...', 'ai-seo-pro' ); ?>');

		$.post(ajaxurl, {
			action: 'ai_seo_pro_flush_sitemap',
			nonce: '<?php echo wp_create_nonce( 'ai_seo_pro_sitemap' ); ?>'
		}, function(response) {
			$btn.prop('disabled', false).text('<?php esc_html_e( 'Flush Sitemap Cache', 'ai-seo-pro' ); ?>');
			if (response.success) {
				alert('<?php esc_html_e( 'Sitemap cache flushed successfully!', 'ai-seo-pro' ); ?>');
			}
		});
	});
});
