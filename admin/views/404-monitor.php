...
// Get logs.
$results = $this->monitor_404->get_404_logs(
	array(
		'page'     => $page,
		'per_page' => 20,
		'search'   => $search,
	...
	<div class="stats-grid"
		...
		<div class="stat-card"
			...
			<h3 style="margin: 0 0 10px 0; color: #666; font-size: 14px;">
				<?php esc_html_e( 'Total 404 Errors', 'ai-seo-pro' ); ?></h3>
			<div style="font-size: 36px; font-weight: bold; color: #dc3232;">
				<?php echo esc_html( number_format( $stats['total_404s'] ) ); ?>
			</div>
		...
		<div class="stat-card"
			...
			<h3 style="margin: 0 0 10px 0; color: #666; font-size: 14px;"><?php esc_html_e( 'Unique URLs', 'ai-seo-pro' ); ?>
			</h3>
			<div style="font-size: 36px; font-weight: bold; color: #ffb900;">
				<?php echo esc_html( number_format( $stats['unique_urls'] ) ); ?>
			</div>
			<p style="margin: 5px 0 0 0; color: #999; font-size: 12px;">
						...
						<tr>
							...
							<td>
								<span
									...
									<?php echo esc_html( number_format( $top->total_hits ) ); ?>
								</span>
							</td>
							<td>
		...
		<form method="get" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
			...
			<label>
				...
				<select name="days" onchange="this.form.submit()">
					<option value="7" <?php selected( $days, 7 ); ?>><?php esc_html_e( 'Last 7 days', 'ai-seo-pro' ); ?>
					</option>
					<option value="30" <?php selected( $days, 30 ); ?>><?php esc_html_e( 'Last 30 days', 'ai-seo-pro' ); ?>
					</option>
					<option value="90" <?php selected( $days, 90 ); ?>><?php esc_html_e( 'Last 90 days', 'ai-seo-pro' ); ?>
					</option>
					<option value="0" <?php selected( $days, 0 ); ?>><?php esc_html_e( 'All time', 'ai-seo-pro' ); ?></option>
				</select>
			</label>
			...
			<div style="margin-left: auto;">
				<a href="<?php echo esc_url( wp_nonce_url( '?page=' . $this->plugin_name . '-404-monitor&action=export_404&days=' . $days, 'export_404_logs' ) ); ?>"
					class="button">
						...
						<tr>
							...
							<td>
								<span style="font-weight: 600; color: #dc3232;">
									<?php echo esc_html( number_format( $log->hits ) ); ?>
								</span>
							</td>
							<td>
								<?php if ( $log->referrer ) : ?>
								...
								<span style="font-size: 12px; color: #666;">
									...
									echo esc_html(
										sprintf(
											...
											__( '%s ago', 'ai-seo-pro' ),
											human_time_diff( strtotime( $log->updated_at ), current_time( 'timestamp' ) )
										)
									);
									?>
								</span>
							...
							<td>
								<button class="button button-small create-redirect-btn"
									data-source="<?php echo esc_attr( $log->url ); ?>" data-log-id="<?php echo esc_attr( $log->id ); ?>">
									<?php esc_html_e( 'Create Redirect', 'ai-seo-pro' ); ?>
								</button>
								<a href="<?php echo esc_url( wp_nonce_url( '?page=' . $this->plugin_name . '-404-monitor&action=delete&id=' . $log->id, 'delete_404_' . $log->id ) ); ?>"
